...
const AllAdvocate = () => {
    ...
    const handleFetchUser = async () => {
        try {
            ...
            const formattedUsers = filtered.filter((user) => user?.role === "notary");
    ...
    return (
        <>
            {loading ? (
                ...
                <Table
                    heading="All Advocate"
                    tableHeading={heading}
                    btnText="Add Advocate"
                    btnURL="/advocate/add-advocate"
                    tableContent={users.map((user, index) => (
                        <CTableRow key={user?._id}>
                            ...
                            <CTableDataCell>
                                ...
                                />
                                <small
                                    className={`d-block mt-1 ${user?.userIdImageVerify
                                        ? "text-success"
                                        : "text-warning"
                                        }`}
                                >
                            ...
                            <CTableDataCell>
                                <CButton
                                    ...
                                    size="sm"
                                    onClick={() =>
                                        window.location.href = `#/advocate/advocate-detail/${user._id}`
                                    }
                                >
                                    View Details
                                </CButton>
